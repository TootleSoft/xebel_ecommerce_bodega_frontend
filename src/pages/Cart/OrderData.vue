<template>
    ...
    <div class="card p-6 bg-gray-100" style="border-radius: 0px">
        <div class="grid grid-nogutter bg-white" style="border-radius: 16px">
            <!-- Izquierda -->
            <div class="col-12 px-4 mt-4 md:mt-6 md:px-6">
                <span class="text-900 block font-bold text-4xl" style="font-family: 'Montsemibold';">Detalle del pedido</span>
            </div>
            <div class="col-12 lg:col-6 h-full px-4 py-4 md:px-6">
                <div class="card flex justify-content-center p-3 bg-blue-900">
                    <div class="flex flex-wrap gap-3" style="font-family: 'Montserrat';">
                        <!-- <div v-if="pickup == 1" class="flex align-items-center">
                            ...
                            <label for="dt1" class="ml-2 text-white">Entrega en Sucursal</label>
                        </div>
                        ...
                        <div v-if="delivery == 1" class="flex align-items-center">
                            ...
                            <label for="dt2" class="ml-2 text-white">Entrega en domicilio</label>
                        </div>
                ...
                <!-- <div v-if="deliveryType == 1" class="grid formgrid" style="border: 4px solid green">
                ...
                <div v-if="deliveryType == 2" class="grid formgrid">
                    <ShippingData v-if="showShippingData" @saveShippingData="refreshReferences"
                        @close="showShippingData = false" :countries="countries" :states="states"></ShippingData>
                    <div v-if="!showShippingData" class="col-12 field mb-1">
                        <span class="text-900 text-2xl block font-medium mb-3">Dirección de entrega</span>
                        <Dropdown :options="customerReferences" v-model="selectedReference"
                            placeholder="Selecciona una dirección de envío" optionLabel="addressText" optionValue="id"
                            showClear class="w-full mb-6" />
                        
                        <div class="col-12 flex align-items-center justify-content-center">
                            <Button v-if="!showShippingData && !selectedReference" @click="showShippingData = true"
                                class="flex align-items-center justify-content-center w-full" label="Agregar nueva dirección"
                                icon="pi pi-fw pi-check"
                                style="background-color: orange; border-color: transparent; color: white;"></Button>
                        </div>
                        <span v-if="selectedReference" class="text-900 text-2xl block font-medium mb-5"
                            style="font-family: 'Montsemibold';">Paqueteria</span>
                        <Dropdown v-if="selectedReference" :options="carriers" v-model="selectedCarrier"
                            placeholder="Selecciona una paquetería" optionLabel="name" optionValue="name" showClear
                            class="w-full" @change="changeCarrier" style="font-family: 'Montserrat'" />
                        <br><br><br>
                        <div v-if="selectedReference && selectedCarrier" class="grid">
                            <div class="col-12 lg:col-4">
                                <span class="text-1500 block font-medium mb-3 font-bold"
                                    style="font-family: 'Montsemibold';">Servicio</span>
                                <ul class="py-0 pl-3 m-0 text-600 mb-3">
                                    <li class="mb-2" v-if="!loading" v-for="(quote, i) in quotations" :key="i"
                                        style="font-family: 'Montserrat'">
                                        <RadioButton v-model="selectedService" :inputId="quote.key" name="dynamic"
                                            :value="quote.service_level_name" @change="serviceChange" />
                                        <label for="quote.key" style="font-family: 'Montserrat';">{{ " " +
                                            quotations[i].service_level_name }}</label>
                                    </li>
                                </ul>
                            </div>
                            <div class="col-12 lg:col-4">
                                <span class="text-1500 block font-medium mb-3 font-bold"
                                    style="font-family: 'Montsemibold'">Entrega</span>
                                <ul class="py-0 pl-3 m-0 text-600 mb-3">
                                    <li class="mb-2" v-if="!blocked" v-for="(quote, i) in quotations" :key="i"
                                        style="font-family: 'Montserrat'">{{ quotations[i].days }} días hábiles</li>
                                </ul>
                            </div>
                            <div class="col-12 lg:col-4">
                                <span class="text-1500 block font-medium mb-3 font-bold"
                                    style="font-family: 'Montsemibold'">Tarifa</span>
                                <ul class="py-0 pl-3 m-0 text-600 mb-3">
                                    <li class="mb-2" v-if="!blocked" v-for="(quote, i) in quotations" :key="i"
                                        style="font-family: 'Montserrat';">${{ quotations[i].total_pricing }}</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- TODO: Agregar función de autocompletar los datos mediante el CP-->
                <div class="grid formgrid">
                    <div class="col-12 field">
                        <div class="field-checkbox">
                            <Checkbox name="checkbox-2" v-model="billable" binary inputId="checkbox-2"/>
                            <label for="checkbox-2" style="cursor: pointer; font-family: 'Montserrat';">¿Desea
                                facturar?</label>
                        </div>
                    </div>
                    <div v-if="billable" class="grid formgrid col-12">
                        <div class="col-12 lg:col-6 field mb-4">
                            <Dropdown :options="countries" v-model="invoice_entity.country" placeholder="País"
                                optionLabel="name" optionValue="name" showClear class="w-full" />
                        </div>
                        <div class="col-12 lg:col-6 field mb-4">
                            <input v-model="invoice_entity.state" id="Estado" placeholder="Estado" type="text"
                                class="p-inputtext w-full" />
                        </div>
                        <div class="col-12 lg:col-6 field mb-4">
                            <input v-model="invoice_entity.municipality" id="Municipio" placeholder="Municipio"
                                type="text" class="p-inputtext w-full" />
                        </div>
                        <div class="col-12 lg:col-6 field mb-4">
                            <input v-model="invoice_entity.suburb" id="Colonia" placeholder="Colonia" type="text"
                                class="p-inputtext w-full" />
                        </div>
                        <div class="col-12 field mb-4">
                            <input v-model="invoice_entity.address" id="Direccion" placeholder="Dirección" type="text"
                                class="p-inputtext w-full" />
                        </div>
                        <div class="col-12 lg:col-6 field mb-4">
                            <input v-model="invoice_entity.postal_code" id="pc" placeholder="Código Postal"
                                type="number" class="p-inputtext w-full" min="0" maxlength="5"/>
                        </div>
                        <div class="col-12 lg:col-6 field mb-4">
                            <input v-model="invoice_entity.rfc" id="rfc" placeholder="RFC" type="text"
                                class="p-inputtext w-full" />
                        </div>
                        <div class="col-12 lg:col-6 field mb-4">
                            <Dropdown :options="useCfdi" v-model="invoice_entity.customerUseCfdi" placeholder="Uso CFDI"
                                optionLabel="Descripcion" optionValue="c_UsoCFDI" showClear class="w-full" />
                        </div>
                        <div class="col-12 lg:col-6 field mb-4">
                            <Dropdown :options="taxRegime" v-model="invoice_entity.customerTaxRegime"
                                placeholder="Régimen Fiscal" optionLabel="Descripcion" optionValue="c_RegimenFiscal"
                                showClear class="w-full" />
                        </div>
                        <div class="col-12 flex align-items-center justify-content-center">
                            <Button @click="saveShippingData" class="flex align-items-center justify-content-center"
                                label="Guardar/Modificar Datos de facturación" icon="pi pi-fw pi-save"
                                style="background-color: #007bff; border-color: #007bff; color: white;"></Button>
                        </div>
                    </div>
                </div>
            </div>
            <!-- Derecha -->
            <div class="col-12 lg:col-6 px-4 py-1 md:px-4" 
            style="
            display: flex; 
            flex-direction: column; 
            border-left: 1px solid #ccc;">
                <div class="mb-2 surface-border">
                ...
                <div style="width: 100%; text-align: right; margin-top: 2rem;">
                    <span class="text-900 block font-bold text-xl" style="font-family: 'Montsemibold';">Total del pedido
                        ${{
                        total.toFixed(2) }}</span><br>
                    <span v-if="deliveryType == 2" class="text-900 block font-bold text-xl"
                        style="font-family: 'Montsemibold';">Envío a domicilio ${{ totalShipment.toFixed(2)
                        }}</span><br>
                    <span class="text-900 block font-bold text-xl" style="font-family: 'Montsemibold';">Total a pagar
                        ${{
                            (Number(total.toFixed(2)) + Number(totalShipment.toFixed(2))).toFixed(2) }}</span>
                </div>
                <div class="col-12 flex flex-column lg:flex-row justify-content-center align-items-center lg:justify-content-end mt-4"
                    style="align-items: flex-end; padding-right: 0">

                    <Button class="mt-0 lg:mt-0 w-full lg:w-auto flex-order-2 lg:flex-order-1 lg:mr-2"
                        severity="secondary" label="Regresar al carrito" icon="pi pi-fw pi-arrow-left"
                        @click="router.push('/shoppingcart');" style="
      ...
      <label for="termsCheckbox" style="font-size: 16px; font-weight: 800; color: #1155cc;">
        <span @click="router.push('/TermsAndConditions')" style="cursor: pointer; color: #1155cc;"> Acepto los términos y condiciones</span>
      </label>
...
const refresh = async () => {
    try {
        ...
        let response = await axios.get(`/Ecommerce/Customer/addresses/${authStore.id_usuario}`)
...
const processPaymentStore = async () => {
    try {
        ...
        let order = await createOrder(1);  // se crea el pedido de tipo 1 porque es un pago en sucursal
...
const processPaymentTransfer = async (type: any) => {
    try {
        ...
        // Verificamos que si hay pedido con envío que también incluya la dirección de envío, sino manda un mensaje de error
        ...
        // Verificamos que los datos de facturación estén completos
        let inn = await invoiceNotNull();
        // Si la facturación está seleccionada, que los datos de facturación estén completos
        if (billable.value == true) {
            if (inn == false) {
                throw "Favor de llenar todos los datos de facturacion"
            }
        }
        // Verificamos que el carrito tenga un pedido
        if (cartStore.order.length == 1) {
            // console.log('Entró al cartStore.order.Length == 1')
            ...
            if (deliveryType.value == 2 && (selectedCarrier.value == null || selectedService.value == null)) {
                ...
                if (response.data && response.data.payment_method) {
                    const clabe = response.data.payment_method.clabe; // Guardamos la CLABE en la variable clabe para pasarlo al componente ConfirmationPage.vue
                    ...
                    console.warn("payment_method no está presente:", response.data);
                }

                let status = response.data.status;
                let openpay_id = response.data.id;
                await axios.post(
                    'Comercial/ECommerceOrder/updateOrder/' +
                    response.data.order_id + '/' +
                    openpay_id + '/' +
                    status +
                    '/' + ''
                );
                // Guarda el id_tracking del pedido
...
const generateShipment = async () => {
    ...
    // Aqui se guardan los parametros de dimensiones en el array parcels y usarlos para el envio
    ...
    const shipment: Shipment = {
        address_from: {
            province: "Nuevo Leon",
            city: "Monterrey",
            name: "José Elizondo Valero",
            zip: "64460",
            country: "MX",
            address1: "Lerma #2056, Mitras Centro, Monterrey",
            company: "BodegadeRemate.com",
            address2: "Centro",
            phone: "",
            email: "bodegaderemate@bodegaderemate.com",
        },
